...
var db = mongojs('CMS',['settings','pageTypes','filters']);
...
router.get('/',function(req,res){
	...
	db.pageTypes.find(function(err,pageTypes){
		
		db.filters.find({},{name:1,niceName:1},function(err,filters){
			req.session.pageTypes = pageTypes;
...
router.post('/page',function(req,res){
	...
	var page = req.body,
		referer = req.headers.referer + '#pages';
	
	if(!page.pageType){
		req.flash('error','Please Chose Page Type');
	...
	db.pageTypes.findOne( {pageType:page.pageType}, function(err,doc){
		if(doc){	
			...
			db.pageTypes.insert(page,function(err, doc){
				res.redirect(referer);
			});
		} 
	});       
...
router.get('/deletePage/:pageType',function(req, res){
	var referer = req.headers.referer + '#pages';
	db.pageTypes.remove({pageType:req.params.pageType}, function(){
		res.redirect(referer);
	});
});
...
router.post('/filter',function(req, res){
	var filter = req.body,
		referer = req.headers.referer + '#filters';
	
	if(!filter.name){
		req.flash('error','Please Chose Filter Name');
		return res.redirect(referer);
	}
	...
	db.filters.findOne({name:filter.name},function(err, doc){
		...
		if(doc){
			...
			filter.list = [];
			db.filters.insert(filter,function(err,doc){
				res.redirect(referer); 
			}); 
		} 
...
router.get('/deleteFilter/:filter',function(req,res){
	var referer = req.headers.referer + '#filters';
	db.filters.remove({name:req.params.filter}, function(){
		res.redirect(referer);
	});
}); 
